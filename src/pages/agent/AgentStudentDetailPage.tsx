...
import { fetchAgentStudentDetail, openAgentDocument } from '../../lib/api'
...
export default function AgentStudentDetailPage() {
  ...
  const detailQuery = useQuery({
    queryKey: ['agent', 'student-detail', pid],
    queryFn: () => fetchAgentStudentDetail(pid as string),
    enabled: !!pid,
  })
    ...
    return (
      <PageWrapper className="space-y-6">
        <EmptyState
          ...
          action={<Button onClick={() => navigate('/portal/agent/students')}>Back to Students</Button>}
        />
      </PageWrapper>
    )
  }
  ...
  const applicationColumns: ColumnDef<any>[] = [
    { key: 'reference', header: 'Application ID', cell: (row) => <span className="font-semibold text-brand-navy">{row.reference_number}</span> },
  ...
  return (
    <PageWrapper className="space-y-6">
      <button
        ...
        onClick={() => navigate('/portal/agent/students')}
      >
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="text-sm text-right">
              <p className="text-muted-foreground text-xs uppercase font-bold tracking-wider">Assigned Agent</p>
